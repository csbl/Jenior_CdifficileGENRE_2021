...
MET_COMPART_KEYS = {"compartment", "Compartment"}
...
RXN_SUBSYS_KEYS = {"subsystem", "Subsystem"}
RXN_OBJ_KEYS = {"objective", "Objective"}
...
def read_excel(
    ...
    if met_sheet_name is not None and met_sheet_name != "ignore":
        ...
        if met_compartment_key is None:
            met_compartment_key = guess_name(met_frame.keys(), MET_COMPART_KEYS,
                                      fail=False)
        ...
        for i in met_frame.index:
            ...
            met = Metabolite(met_id,
                             name=extract(met_row, met_name_key),
                             **met_attributes)
            met.compartment = extract(met_row, met_compartment_key)
            try:
    ...
    if rxn_obj_key is None:
        rxn_obj_key = guess_name(rxn_frame.keys(), RXN_OBJ_KEYS, fail=False)
        if verbose and rxn_obj_key is None:
            print("objective function column not identified")
    ...
    if rxn_subsys_key is None:
        rxn_subsys_key = guess_name(rxn_frame.keys(), RXN_SUBSYS_KEYS)

    for i in range(len(rxn_frame)):
        rxn_row = rxn_frame.ix[i]
        if rxn_id_key == "auto":
            rxn_id = "R%04d" % i
        else:
            rxn_id = extract(rxn_row, rxn_id_key)
        rxn_str = extract(rxn_row, rxn_str_key)
        # skip empty rows
        ...
        rxn = Reaction()
        rxn.id = rxn_id
        rxn.name = extract(rxn_row, rxn_name_key)
        rxn.subsystem = extract(rxn_row, rxn_subsys_key)
        ...
        gpr = extract(rxn_row, rxn_gpr_key)
        if "," in gpr or "+" in gpr:
        ...
        if rxn_lb_key is not None:
            rxn.lower_bound = float(rxn_row[rxn_lb_key])
        if rxn_ub_key is not None:
            rxn.upper_bound = float(rxn_row[rxn_ub_key])

        # Set objective function
        objective = extract(rxn_row, rxn_obj_key)
        if objective == '1':
            m.objective = rxn
