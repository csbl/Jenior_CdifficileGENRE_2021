...
lb_aerobic_samples <- read.delim(file='/home/mjenior/Desktop/repos/Cdiff_modeling/data/flux_samples/LB_aerobic.flux_samples.format.tsv', sep='\t', header=TRUE, row.names=1)
colnames(lb_aerobic_samples) <- make.names(colnames(lb_aerobic_samples))
...
#m9_aerobic_samples <- read.delim(file='/home/mjenior/Desktop/repos/Cdiff_modeling/data/flux_samples/M9_aerobic.flux_samples.format.tsv', sep='\t', header=TRUE, row.names=1)
#colnames(m9_aerobic_samples) <- make.names(colnames(m9_aerobic_samples))
#m9_aerobic_samples <- m9_aerobic_samples[small_sample,]
...
#m9_anaerobic_samples <- read.delim(file='/home/mjenior/Desktop/repos/Cdiff_modeling/data/flux_samples/M9_anaerobic.flux_samples.format.tsv', sep='\t', header=TRUE, row.names=1)
#colnames(m9_anaerobic_samples) <- make.names(colnames(m9_anaerobic_samples))
#m9_anaerobic_samples <- m9_anaerobic_samples[small_sample,]
...
#write.table(lb_invivo_mda, file='/home/mjenior/Desktop/repos/Cdiff_modeling/data/flux_samples/rf_features.tsv', row.names=FALSE, quote=FALSE, sep='\t')
...
lb_invivo_mda <- read.delim(file='/home/mjenior/Desktop/repos/Cdiff_modeling/data/flux_samples/rf_features.tsv', sep='\t', header=TRUE)
...
samples_pcoa_points <- read.delim('~/Desktop/invitro_invivo_pcoa.tsv', header=TRUE)
x_axis_lab <- gsub('X', '', as.character(colnames(samples_pcoa_points)[1]))
x_axis_lab <- paste ('PC1 (', x_axis_lab, '%)', sep='')
y_axis_lab <- gsub('X', '', as.character(colnames(samples_pcoa_points)[2]))
y_axis_lab <- paste ('PC2 (', y_axis_lab, '%)', sep='')
...
# Generate figure
library(scales)
#png(filename='~/Desktop/repos/Cdiff_modeling/results/figure_5.png', units='in', width=7, height=8, res=300)
pdf(file='~/Desktop/repos/Cdiff_modeling/results/figure_5.pdf', width=7, height=8)
layout(matrix(c(1,1,2,2,2,
                3,3,4,4,4), nrow=2, ncol=5, byrow=TRUE))

# A
par(mar=c(4,4,1,1), las=1, mgp=c(2.6,1,0), xaxs='i', yaxs='i', lwd=2)
...
text(x=11.4, y=c(4,-4), c('Increased products','Increased reactants'), srt=270, cex=0.9)
