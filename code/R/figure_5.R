...
select_and_plot <- function(metabolite_name, best_ylim=0, correction=FALSE, title=NA, panel=FALSE) {
    ...
    par(mar=c(3,3,1.5,0.5), xpd=FALSE, mgp=c(1.9,0.7,0), lwd=1.7, xaxt='n', las=1)
    boxplot(a_mock, at=1, xlim=c(0.6,3.4), ylim=c(0,best_ylim), col='firebrick2', cex.axis=0.8,
            xlab="", ylab="Scaled Intesity", outcex=0, whisklty=1, medlwd=2,  main=title, cex.main=1) 
    ...
    boxplot(b_630, at=3, xlim=c(0.6,3.4), ylim=c(0,best_ylim), col='blue3', 
            xlab="", ylab="Scaled Intesity", outcex=0, whisklty=1, medlwd=2, yaxt='n',add=TRUE) 
    mtext(c('High','Low'), side=1, at=c(1.25,2.75), padj=2)
    ...
    if (a_pval <= 0.05) {
        segments(x0=1, y0=best_ylim*0.96, x1=1.5, y1=best_ylim*0.96, lwd=1.7)
...
png(filename='~/Desktop/repos/Jenior_Cdifficile_2019/results/figures/figure_5A.png', 
...
png(filename='~/Desktop/repos/Jenior_Cdifficile_2019/results/figures/figure_5B1.png', 
    units='in', width=4, height=6, res=300)
par(mar=c(2.5, 0.5, 0.5, 0.5), mgp=c(1.4, 0.5, 0), xpd=FALSE, yaxs='i', lwd=1.7)
plot(1, type='n', ylim=c(0.5,18.5), xlim=c(-30,30), 
     ylab='', xlab='Sampled Reaction Flux', xaxt='n', yaxt='n', cex.lab=0.9)
par(xaxt='s')
axis(1, at=c(-30,-15,0,15,30), cex.axis=0.8) 
abline(v=0, col='gray', lwd=1.7)
box(lwd=1.7)
text(x=-24, y=2.9, labels='OOB Error = 0%', cex=0.7)
legend('bottomleft', legend=c('Low Sporulation','High Sporulation'), 
       col=c('blue3','firebrick2'), pch=15, pt.cex=1.7, box.lwd=1.7)
index <- 2
for(i in c(1:ncol(rf_clinda_samples))){
    stripchart(at=index+0.4, rf_strep_samples[,i], 
               pch=15, col=alpha('blue3',0.8), cex=2.2, add=TRUE)
    stripchart(at=index-0.8, rf_clinda_samples[,i], 
               pch=15, col=alpha('firebrick2',0.8), cex=2.2, add=TRUE)
    if (i != ncol(rf_clinda_samples)){
        abline(h=index+1.5, lty=2)
    }
    # Medians
    segments(median(rf_strep_samples[,i]), index+0.8, median(rf_strep_samples[,i]), index, lwd=2)
    segments(median(rf_clinda_samples[,i]), index-1.2, median(rf_clinda_samples[,i]), index-0.4, lwd=2)
    index <- index + 3
}
text(x=-0.5, y=c(3,6,9,12,15,18), labels=top_rf_rxns$label, cex=0.7, pos=4)
dev.off()

# Flux samples from AUCRF features
library(plotrix)
png(filename='~/Desktop/repos/Jenior_Cdifficile_2019/results/figures/figure_5B2.png', 
...
text(x=9, y=seq(1.3,10.3,1), labels=rf_rxns$name, cex=0.7, pos=4)
dev.off()

# Metabolite concentrations
png(filename='~/Desktop/repos/Jenior_Cdifficile_2019/results/figures/figure_5D.png', 
    units='in', width=4, height=6, res=300)
layout(matrix(c(1,2,
